    ...
    const XeonBotInc = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
            ...
            return {
                conversation: "JusticeTechBot Is Here!"
            }
        },
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2349032578690")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +2349032578690 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: JusticeTech`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: JusticeTechBot`,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @iam_justicetech`,'magenta'))
...
"sourceUrl": `${wagc}`}}})
                	...
                	const xeonbuffer = await getBuffer(ppuser)
                    const xeontime = moment.tz('Africa/Nigeria').format('HH:mm:ss')
	                const xeondate = moment.tz('Africa/Nigeria').format('DD/MM/YYYY')
                	let xeonName = num
